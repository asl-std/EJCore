package ru.asl.api.bukkit.redstone;
...
public abstract class RedstoneParts {
	...
	public static boolean blockWillBePowered(Block from) {
		...
		return !powered.stream().filter(b -> (b.isBlockPowered() || Core.getRedstoneParts().isEnabledRedstone(b.getType()))).collect(Collectors.toList()).isEmpty();
	}
